...
public abstract class SchemaMessageReader<T> implements MessageReader {
  ...
  public void init(java.io.InputStream inputStream, Properties props) {
    ...
    if (props.containsKey("parse.headers")) {
      parseHeaders = props.getProperty("parse.headers").trim().toLowerCase().equals("true");
    }
  ...
  public ProducerRecord<byte[], byte[]> readMessage() {
    try {
      ...
      if (parseHeaders) {
        int headersIndex = line.indexOf(headersDelimiter);
        if (headersIndex < 0) {
          if (!ignoreError) {
            throw new KafkaException("No header found in line " + line);
          }
        } else {
          // try the json way
          try {
            String jsonString = line.substring(0, headersIndex);
            new ObjectMapper().readTree(jsonString).fields().forEachRemaining(e -> {
              String keyString = e.getKey();
              String valueString = e.getValue().asText();
              headers.add(new RecordHeader(keyString, valueString.getBytes()));
            });
          } catch (JsonProcessingException e) {
            // does not parse, let's do the delimiter way
            String headersString = line.substring(0, headersIndex);
            for (String h : headersString.split(headersSeparator)) {
              int keyIndex = h.indexOf(headersKeySeparator);
              if (keyIndex < 0) {
                if (ignoreError) {
                  continue;
                }
                throw new KafkaException("No header key found in line " + line);
              }
              String keyString = h.substring(0, keyIndex);
              String valueString = h.substring(keyIndex + headersKeySeparator.length());
              headers.add(new RecordHeader(keyString, valueString.getBytes()));
            }
          }
          line = line.substring(headersIndex + headersDelimiter.length());
        }
      }
      ...
      if (parseKey) {
        ...
        line = line.substring(keyIndex + keySeparator.length());
      }
      T value = readFrom(line, valueSchema);
      byte[] serializedValue = serializer.serialize(valueSubject, topic, false, value, valueSchema);
